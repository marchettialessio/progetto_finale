...
namespace monopoly
{
	...
	void play_bot(GameBoard &current_gameboard, Player *player)
	{
		...
		// check if it passes from the start
		...
		// if it wasn't for sale and you're not the box owner
		if (!dynamic_cast<LateralBox *>(current_box)->is_box_owner(player))
		{
			...
			Log::log_overnight_stay(player->get_name(), dynamic_cast<LateralBox *>(current_box)->get_owner()->get_name(),
									...
									dynamic_cast<LateralBox *>(current_box)->get_name());
		...
		// if it wasn't for sale and you're the box owner
